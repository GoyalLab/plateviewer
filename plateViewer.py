...
class PlateViewer(QWidget):
    def __init__(self):
        ...
        # Export button (moved here, renamed)
        export_btn = QPushButton("Export Annotations to CSV")
        export_btn.clicked.connect(self.export_markings_to_csv)
        self.top_bar.addWidget(export_btn)
    ...
    def export_markings_to_csv(self):
        """Export well markings to a CSV file."""
        import csv
        from PyQt5.QtWidgets import QFileDialog
        
        if not self.checked_wells:
            from PyQt5.QtWidgets import QMessageBox
            QMessageBox.information(self, "No Data", "No wells have been marked yet.")
            return
        ...
        try:
            with open(filename, 'w', newline='') as csvfile:
                ...
                for (plate, well), marking in self.checked_wells.items():
                    # Extract plate number from plate name (e.g., "PLATE1" -> "1")
                    plate_number = plate.replace("PLATE", "").replace("plate", "")
                    clone_id = f"{plate_number}{well}"  # e.g., "1A1", "2C3"
                    writer.writerow([clone_id, marking])
            
            from PyQt5.QtWidgets import QMessageBox
            QMessageBox.information(self, "Export Complete", f"Markings exported to {filename}")
    ...
    def open_detail_view(self, well):
        ...
        timepoints = sorted(set(d["timepoint"] for d in plate_images))
        if not timepoints:
            return  # No images for this well
        self.current_timepoint = timepoints[-1]  # Use the latest timepoint
    ...
    def cache_well_images(self, well):
        ...
        key = (self.current_plate, well)
        if key in self.well_cache:
            return  # Already cached

        self.well_cache[key] = {}  # Mark as being cached
        self.start_caching_thread(well)

    def update_thumbnail_cache(self, cached_data):
        self.thumbnail_cache.update(cached_data)
        # If we're currently viewing a well and new images are cached, refresh the display
        if self.current_well and self.stacked_layout.currentWidget() == self.detail_widget:
            self.display_detail_image()
    ...
    def preload_next_wells(self, num_wells=5):
        ...
        for offset in range(1, num_wells + 1):
            if idx + offset < len(all_wells):
                ...
                key = (self.current_plate, next_well)
                if key not in self.well_cache:
                    #print(f"Preloading well: {next_well} for plate {self.current_plate}")
                    self.cache_well_images(next_well)
                #else:
                    #print(f"Well {next_well} for plate {self.current_plate} is already cached.")
    ...
    def update_plate(self):
        ...
        # Automatically switch back to plate view when a different plate is selected
        self.stacked_layout.setCurrentWidget(self.scroll_area)
        self.right_panel_widget.hide()  # Hide the right panel as well

        self.update_grid()
        #self.start_plate_loading_thread()  # <-- Only call here!

        # Automatically cache well A1 when switching to a new plate
        if self.current_plate and self.image_data:
            # Check if A1 has images for this plate
            a1_images = [d for d in self.image_data if d["plate"] == self.current_plate and d["well"] == "A1"]
            if a1_images:
                #print(f"Auto-caching well A1 for plate {self.current_plate}")
                self.cache_well_images("A1")
            #else:
                #print(f"No images found for well A1 in plate {self.current_plate}")
    ...
    def update_grid(self):
        ...
        # Add clickable well name buttons in a grid with markings
        for i, row in enumerate("ABCDEFGH"):
            for j in range(1, 13):
                ...
                # Check if this well has a marking
                key = (self.current_plate, well)
                marking = self.checked_wells.get(key, None)
                
                # Set style based on marking
                if marking == "singlet":
                    btn.setStyleSheet(
                        "color: white; background-color: #222; font-weight: bold; font-size: 14px; "
                        "border: 3px solid blue;"
                    )
                elif marking == "doublet":
                    btn.setStyleSheet(
                        "color: white; background-color: #222; font-weight: bold; font-size: 14px; "
                        "border: 3px solid red;"
                    )
                elif marking == "inconclusive":
                    btn.setStyleSheet(
                        "color: white; background-color: #222; font-weight: bold; font-size: 14px; "
                        "border: 3px solid orange;"
                    )
                else:
                    # Default style (no marking)
                    btn.setStyleSheet(
                        "color: white; background-color: #222; font-weight: bold; font-size: 14px; "
    ...
    def display_detail_image(self, is_gfp=False):
        ...
        if self.zoom_state is not None:
            ...
            self.zoom_state = None  # Reset after use
